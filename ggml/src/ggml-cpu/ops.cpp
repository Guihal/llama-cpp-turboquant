...
// ggml_compute_forward_fused_ffn
// spec 041 T7: CPU fallback for fused FFN. Decomposes the 4-src op into a
// 5-node subgraph (gate@x, silu, up@x, mul, down@scratch) and computes it on a
// fresh CPU sub-context, then copies the [hidden, n_tokens] result into dst.
// Mirrors the Vulkan fused_ffn.comp two-pass math; serves as the correctness
// reference/fallback. Inference-only V1 (no backward).
